...
public class Estacion5 extends Thread {
    ...
    public Estacion5(ProcesoEstacion datos, RecursosCompartidos recursosCompartidos) {
        this.datos = datos;
    ...
    public void run() {
        ...
        final int ESTACION_ANTERIOR = 4;
        final int ESTACION_ACTUAL = 5;
        final int ESTACION_SIGUIENTE = 6;
    ...
    private void procesoLlantas(int autoActual) {
        int linea = rc.getNumeroLinea() - 1;
        Robot robot = obtenerRobot();
        String urlCarroceria = "imagenes/llantas.png";
        Vista.actualizarInterfaz(linea, 11, autoActual, urlCarroceria, robot.getUrlImagen());
        Rutinas.dormirHilo(datos.getTiempoEspera());
        Vista.quitarInterfaz(linea, 11);
        liberarRobot(robot);
    }
