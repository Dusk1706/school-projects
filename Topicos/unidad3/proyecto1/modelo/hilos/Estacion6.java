...
public class Estacion6 extends Thread {
    ...
    public Estacion6(ProcesoEstacion datos, RecursosCompartidos recursosCompartidos) {
        this.datos = datos;
    ...
    public void run() {
        ...
        final int ESTACION_ANTERIOR = 5;
        final int ESTACION_ACTUAL = 6;
    ...
    private void procesoPruebas(int autoActual) {
        int linea = rc.getNumeroLinea() - 1;
        Robot robot = obtenerRobot();
        String urlCarroceria = "imagenes/pruebas.png";
        Vista.actualizarInterfaz(linea, 13, autoActual, urlCarroceria, robot.getUrlImagen());
        Rutinas.dormirHilo(datos.getTiempoEspera());
        Vista.quitarInterfaz(linea, 13);
        liberarRobot(robot);
    }
